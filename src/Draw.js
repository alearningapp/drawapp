...
const Draw = () => {
  ...
  const curCanvasRef = useRef(null);
  ...
  const setDrawingStyle = (ctx) => {
    ...
    ctx.strokeStyle = `rgba(${hexToRgb(setting.color).r}, ${
      hexToRgb(setting.color).g
    }, ${hexToRgb(setting.color).b}, ${setting.opacity})`;
  ...
  const startDrawing = useCallback((e) => {
    ...
    // Restore previous paths on the main canvas
    restoreCanvas();

    const ctx = curCanvasRef.current.getContext("2d");
    ...
    // Clear the current drawing canvas
    const curCtx = curCanvasRef.current.getContext("2d");
    curCtx.clearRect(
      0,
      0,
      curCanvasRef.current.width,
      curCanvasRef.current.height
    );
  }, []);

  const moveDraw = (e) => {
    const { offsetX, offsetY } = getOffset(e);
    const rect = curCanvasRef.current.getBoundingClientRect();
  ...
  const getOffset = (e) => {
    const rect = curCanvasRef.current.getBoundingClientRect();
    const scaleX = curCanvasRef.current.width / rect.width;
    const scaleY = curCanvasRef.current.height / rect.height;
    let x, y;
    if (e.touches) {
  ...
  const restoreCanvas = () => {
    ...
    for (let i = 0; i <= currentIndexRef.current; i++) {
      ...
      if (points) {
        ctx.lineWidth = width;
        ctx.strokeStyle = `rgba(${hexToRgb(color).r}, ${hexToRgb(color).g}, ${
          hexToRgb(color).b
        }, ${opacity})`;
  ...
  useEffect(() => {
    ...
    curCanvas.width = canvas.width = wrapper.clientWidth;
    curCanvas.height = canvas.height = wrapper.clientHeight;
    restoreCanvas();

    const handleResize = () => {
      const newWidth = wrapper.clientWidth;
      const newHeight = wrapper.clientHeight;
      curCanvas.width = canvas.width = newWidth;
      curCanvas.height = canvas.height = newHeight;
      restoreCanvas();
    };
